...
この節はアプリ版1.0.0のビルド2・3・4に適用します。ビルド1にはMP4変換とSwift変換接続コードを含みません。ビルド2は旧提出候補、ビルド3は初回ストア提出版、ビルド4は再審査用ビルドで、同じ動画ネイティブ構成を用います。ビルド3・4には本体側の後続修正があります。これらの資料は、一般公開済みまたはAppleの審査承認済みであることを意味しません。`version_probe.c` と `mpv_no_subtitles.c` は8月31日から使用する共通部品です。
...
## 2026-09-12 — 1.0.0 (4) の再審査用資料

ビルド4用の再リンク材料を追加します。動画ネイティブ部品はビルド3と同じで、追加のパッチや実装変更はありません。本体側にはサムネイル初期OFF・初期NG・NG巻き込み修正を含みます。資料の対象版説明をビルド4へ対応させ、過去の固定公開資料は保持します。照合方法と実測結果は同梱 `MANIFEST.json` を確認してください。
