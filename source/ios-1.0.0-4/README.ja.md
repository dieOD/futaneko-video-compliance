...
対象のアプリ版・ビルド番号は `MANIFEST.json` を確認してください。ビルド1はMP4出力追加前のTestFlight版、ビルド2はMP4出力を含む旧提出候補、ビルド3は初回ストア提出版、ビルド4は再審査用ビルドです。以下のMP4・Swift接続コード・試験素材の項目は、アプリ版1.0.0のビルド2・3・4に含まれます。ビルド3・4には本体側の後続修正がありますが、動画ネイティブ部品の構成はビルド2と同じです。ビルド4はサムネイル初期OFF・初期NG・NG巻き込み修正を含みます。資料の提供は、一般公開済みまたはAppleの審査承認済みであることを意味しません。
...
tool/ios_video/
  ...
  audit_ios_video.sh
  patches/
  mp4_export.m         # 1.0.0のビルド2・3・4
  version_probe.c
  mpv_no_subtitles.c
  probe_library_relink.sh、relink_probe.patch  # 1.0.0のビルド2・3・4
  test_mp4_export.sh、mp4_export_test.m、test_mp4_export_paths.sh、mp4_export_paths_test.swift  # 1.0.0のビルド2・3・4
packages/media_kit_libs_ios_video/
  LICENSE
  ios/Classes/         # 1.0.0のビルド2・3・4
...
ビルド1は保存ソースから再構築し、配布IPA内の版・機能構成・パッチ識別子・公開API・動的依存を照合しています。ただし当時のIPAにソース管理の識別子がなく、全実行バイトが一致する再現ビルドを証明したという意味ではありません。ビルド2・3・4は、同じ動画ネイティブ構成を用いた製品Frameworkとの実装section照合と追加ソースのハッシュ確認を行います。ビルド4では、保管された製品Frameworkが旧版の記録と同じSHA-256であることも確認しています。各版の実測結果と照合方法は同梱の `MANIFEST.json` を確認してください。SDKや作業パスが異なる再ビルドのバイト一致はいずれも保証しません。
