...
create_thin_framework() {
  ...
  local export_object="$target_root/$name/mp4_export.o"
  ...
  "$clang_path" -c "$script_dir/mp4_export.m" -o "$export_object" \
    -target "$target" -isysroot "$sysroot" -O2 -fPIC -fblocks -fno-objc-arc \
    -I"$prefix/include"

  nm -gU "$libmpv" | awk '{ print $NF }' | grep '^_mpv_' | sort -u > "$exports"
  nm -gU "$probe_object" | awk '{ print $NF }' | \
    grep '^_nijineko_video_' | sort -u >> "$exports"
  nm -gU "$export_object" | awk '{ print $NF }' | \
    grep '^_nijineko_video_export_' | sort -u >> "$exports"
  ...
  "$clang_path" -dynamiclib -target "$target" -isysroot "$sysroot" \
    ...
    "${linker_args[@]}" \
    "$probe_object" "$export_object" \
    -framework AVFoundation -framework AudioToolbox -framework CoreAudio \
    -framework CoreFoundation -framework CoreGraphics -framework CoreMedia -framework CoreVideo \
...
build_info="$plugin_ios/build-info.txt"
{
  ...
  echo "mp4_export_source_sha256=$(shasum -a 256 "$script_dir/mp4_export.m" | awk '{print $1}')"
  echo "mp4_export=local-decoder-to-apple-h264-aac (FFmpeg encoders/muxers remain disabled)"
